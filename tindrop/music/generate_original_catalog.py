...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="tindrop_music_") as temporary:
        ...
        for track in TRACKS:
            ...
            if not args.reuse_encoded or not output.is_file():
            ...
            cues = canonical_beat_map(output, duration_ms, args.tindrop_root)
    ...
    if args.catalog.is_file():
        existing_payload = json.loads(args.catalog.read_text(encoding="utf-8"))
    original_ids = {track.id for track in TRACKS}
    preserved_tracks = [
        track
        for track in (existing_payload or {}).get("tracks", [])
        if track.get("id") not in original_ids
    ]
    all_tracks = sorted(
        catalog_tracks + preserved_tracks,
