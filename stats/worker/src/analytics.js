...
function sum(rows, field, start, end) {
  return rowsInWindow(rows, start, end).reduce((total, row) => total + (Number(row[field]) || 0), 0);
}
...
function latestValue(rows, field) {
  return [...rows].reverse().find(row => row[field] !== null && row[field] !== undefined)?.[field] ?? null;
}

function maxValue(rows, field) {
  const values = rows.map(row => row[field]).filter(value => value !== null && value !== undefined).map(Number);
...
function summarizePeriod(days, today, apple, admob, play, reviews, currency) {
  ...
  const previousRevenue = sum(apple, "proceeds", previousStart, previousEnd)
  ...
  const latestCrashRate = latestValue(currentPlay, "userPerceivedCrashRate") ?? latestValue(currentPlay, "crashRate");
  const latestAnrRate = latestValue(currentPlay, "userPerceivedAnrRate") ?? latestValue(currentPlay, "anrRate");
  ...
  return {
    ...
    revenue: {
      ...
      apple: appleRevenue,
      admob: admobRevenue,
      change: currentApple.length || currentAdmob.length || previousApple.length || previousAdmob.length
    ...
    acquisition: {
      ...
      refunds,
      transactions,
      downloadsChange: currentApple.length || previousApple.length ? change(downloads, previousDownloads) : null
    },
    ads: {
      ...
      impressionsChange: currentAdmob.length || previousAdmob.length ? change(impressions, previousImpressions) : null
    },
    quality: {
      crashRate: latestCrashRate,
      anrRate: latestAnrRate,
      peakCrashRate: maxValue(currentPlay, "userPerceivedCrashRate") ?? maxValue(currentPlay, "crashRate"),
      peakAnrRate: maxValue(currentPlay, "userPerceivedAnrRate") ?? maxValue(currentPlay, "anrRate")
    },
    reviews: reviews || { recentCount: 0, averageRating: null, lastReviewAt: null, recent: [] },
    currency
  };
}
...
function mergeDaily(today, apple, admob, play) {
  ...
  return lastDays(60, today).map(date => {
    const appleDay = appleByDate.get(date);
    ...
    return {
      ...
      rpm: admobDay ? Number(admobDay.impressionRpm || 0) : null,
...
function insightsFor(period) {
  ...
  if (period.ads.matchRate !== null && period.ads.matchRate < 0.8) {
    insights.push({ code: "matchRateWarn", tone: "warn", value: period.ads.matchRate, valueType: "percent" });
  } else if (period.ads.matchRate !== null) {
    insights.push({ code: "matchRateGood", tone: "good", value: period.ads.matchRate, valueType: "percent" });
  }
  if (period.quality.crashRate !== null) insights.push(period.quality.crashRate > 0.01
    ? { code: "qualityWarn", tone: "warn", value: period.quality.crashRate, valueType: "percent" }
    : { code: "qualityGood", tone: "good", value: period.quality.crashRate, valueType: "percent" });
  if (period.coverage.appleDays < Math.min(period.days, 7)) {
    insights.push({ code: "appleCoverage", tone: "neutral", value: period.coverage.appleDays });
  }
...
export function buildDashboard(state, viewer = {}) {
  ...
  const apple = appleData.daily || [];
  const admob = admobData.daily || [];
  const play = playData.daily || [];
  const reviews = playData.reviews || null;
  const periods = {
    7: summarizePeriod(7, today, apple, admob, play, reviews, currency),
    30: summarizePeriod(30, today, apple, admob, play, reviews, currency)
  };
  ...
  const hasUnconvertedAppleRevenue = apple.some(day => Object.entries(day.proceedsByCurrency || {})
    .some(([code, amount]) => code !== currency && Number(amount) !== 0));
  const daily = mergeDaily(today, apple, admob, play);
  ...
  return {
    ...
    sources: {
      apple: { ...sourcePublic(state.sources?.apple), availableDays: apple.length, backfillComplete: Boolean(appleData.backfillComplete) },
      play: { ...sourcePublic(state.sources?.play), availableDays: play.length },
      admob: { ...sourcePublic(state.sources?.admob), availableDays: admob.length }
    },
    summary: {
      currency,
      coverage: { apple: apple.length > 0, admob: admob.length > 0, play: play.length > 0, revenue: apple.length > 0 || admob.length > 0 },
    ...
    trends: { revenue: daily.slice(-30).map(day => ({ date: day.date, apple: day.appleRevenue || 0, admob: day.admobRevenue || 0 })) }
  };
}

export const analyticsInternals = { sum, change, latestValue, ratio, summarizePeriod, mergeDaily };
