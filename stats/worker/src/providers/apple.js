...
function summarizeRows(rows, appId, appSku, reportDate, currency) {
  ...
  for (const row of rows) {
    ...
    if (DOWNLOAD_TYPES.has(row["Product Type Identifier"]) && units > 0) downloads += units;
    ...
    proceedsByCurrency[rowCurrency] = (proceedsByCurrency[rowCurrency] || 0) + proceeds;
    const country = row["Country Code"] || "—";
    const countryData = countries[country] || { code: country, downloads: 0, proceeds: 0 };
    if (DOWNLOAD_TYPES.has(row["Product Type Identifier"]) && units > 0) countryData.downloads += units;
...
async function fetchAppleReviews(env) {
  ...
  const response = await fetch(`https://api.appstoreconnect.apple.com/v1/apps/${env.APPLE_APP_ID}/customerReviews?${parameters}`, {
    headers: { Authorization: `Bearer ${await appleToken(env)}`, Accept: "application/json" }
  });
  if (response.status === 403 || response.status === 404) return null;
  if (!response.ok) throw new Error(`App Store reviews returned ${response.status}`);
  ...
  return {
    ...
    averageRating: recent.length ? recent.reduce((total, review) => total + review.rating, 0) / recent.length : null,
...
export async function refreshApple(env, existing = {}) {
  ...
  for (let offset = 1; offset < HISTORY_DAYS && targets.length < BACKFILL_DAYS; offset += 1) {
  ...
  const [points, reviewsResult] = await Promise.all([
    ...
    fetchAppleReviews(env).catch(error => {
      console.warn(JSON.stringify({ event: "apple_reviews_unavailable", message: String(error?.message || error) }));
      return existing.reviews || null;
    })
  ]);
  const replacedDates = new Set(points.map(point => point.date));
  const daily = [...(existing.daily || []).filter(item => !replacedDates.has(item.date)), ...points]
    .sort((left, right) => left.date.localeCompare(right.date))
    .slice(-90);
  const countries = new Map();
  for (const day of daily.slice(-30)) {
    for (const country of day.countries || []) {
      const total = countries.get(country.code) || { code: country.code, downloads: 0, proceeds: 0 };
  ...
  return {
    daily,
    backfillComplete: daily.length >= HISTORY_DAYS,
    countries: [...countries.values()].sort((left, right) => (right.proceeds + right.downloads) - (left.proceeds + left.downloads)).slice(0, 8),
