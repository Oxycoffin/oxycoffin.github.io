...
function collect(results,w,h){
  ...
  for(const rec of recs){
    ...
    if(best>=0&&dist<.24){Object.assign(linearTracks[best],{x:c.x,y:c.y,last:now});used.add(best);continue}
...
function fit(){
  const v=$("video"),c=$("scanCanvas");if(!v.videoWidth||!v.videoHeight)return false;
  const maxSide=scanEngine==="wasm"?1280:1080,sc=Math.min(1,maxSide/Math.max(v.videoWidth,v.videoHeight));
  const w=Math.max(360,Math.round(v.videoWidth*sc)),h=Math.max(270,Math.round(v.videoHeight*sc));
  if(c.width!==w)c.width=w;if(c.height!==h)c.height=h;return true;
}

async function prepareWasm(){
  if(wasmReady)return true;if(wasmInitPromise)return wasmInitPromise;if(!window.ZXingWASM?.readBarcodes)return false;
  const init=(async()=>{try{if(typeof ZXingWASM.prepareZXingModule==="function")await ZXingWASM.prepareZXingModule({overrides:{locateFile:(path,prefix)=>path.endsWith(".wasm")?"https://cdn.jsdelivr.net/npm/zxing-wasm@3.1.1/dist/reader/zxing_reader.wasm":prefix+path},fireImmediately:true});wasmReady=true;return true}catch(e){console.error("ZXing WASM init failed",e);wasmReady=false;return false}})();
  wasmInitPromise=Promise.race([init,new Promise(resolve=>setTimeout(()=>resolve(false),3500))]);return wasmInitPromise;
}

function canonicalBrowserFormat(name){return({DATA_MATRIX:"DataMatrix",EAN_13:"EAN13",EAN_8:"EAN8",UPC_A:"UPCA",UPC_E:"UPCE",CODE_128:"Code128",CODE_39:"Code39",ITF:"ITF"})[name]||name||""}
function browserResultToReadResult(result,ox=0,oy=0,scaleX=1,scaleY=1){
  const text=typeof result?.getText==="function"?result.getText():String(result?.text||"");let rawFormat="";
  try{const f=result.getBarcodeFormat?.();rawFormat=window.ZXingBrowser?.BarcodeFormat?.[f]||String(f??"")}catch{}
  const pts=(result.getResultPoints?.()||[]).map(p=>({x:ox+(p.getX?.()??p.x??0)*scaleX,y:oy+(p.getY?.()??p.y??0)*scaleY}));let position;
  if(pts.length){const xs=pts.map(p=>p.x),ys=pts.map(p=>p.y),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys);position={topLeft:{x:minX,y:minY},topRight:{x:maxX,y:minY},bottomRight:{x:maxX,y:maxY},bottomLeft:{x:minX,y:maxY}}}
  return{text,format:canonicalBrowserFormat(rawFormat),symbology:canonicalBrowserFormat(rawFormat),position,isValid:true};
}
function ensureJsReader(){if(jsReader)return true;if(!window.ZXingBrowser?.BrowserMultiFormatReader)return false;jsReader=new ZXingBrowser.BrowserMultiFormatReader();return true}
...
function decodeCrop(reader,source,sx,sy,sw,sh,w,h,tryInvert=false){
  cropCanvas ||= document.createElement("canvas");
  const cw=Math.max(280,Math.round(w*.66)),ch=Math.max(210,Math.round(h*.66));cropCanvas.width=cw;cropCanvas.height=ch;
  const ctx=cropCanvas.getContext("2d",{willReadFrequently:true});ctx.drawImage(source,sx,sy,sw,sh,0,0,cw,ch);
  try{const r=reader.decodeFromCanvas(cropCanvas);return browserResultToReadResult(r,sx,sy,sw/cw,sh/ch)}catch{}
  if(tryInvert){
    try{invertCanvas(cropCanvas);const r=reader.decodeFromCanvas(cropCanvas);return browserResultToReadResult(r,sx,sy,sw/cw,sh/ch)}catch{}
  }
  return null;
}
function readWithJsFallback(canvas){
  if(!ensureJsReader())return[];const results=[];
  try{const r=jsReader.decodeFromCanvas(canvas);if(r)results.push(browserResultToReadResult(r))}catch{}
  const w=canvas.width,h=canvas.height,patterns=[[0,0,.6,1],[.4,0,.6,1],[0,0,1,.6],[0,.4,1,.6],[.15,.15,.7,.7],[0,0,.65,.65],[.35,0,.65,.65],[0,.35,.65,.65],[.35,.35,.65,.65]];
  for(let n=0;n<4;n++){const p=patterns[(cropIndex+n)%patterns.length],r=decodeCrop(jsReader,canvas,Math.round(p[0]*w),Math.round(p[1]*h),Math.round(p[2]*w),Math.round(p[3]*h),w,h,(frame+n)%5===0);if(r)results.push(r)}
  cropIndex=(cropIndex+4)%patterns.length;return results;
}

function mergeResults(a,b){
  const out=[...a];for(const r of b){if(!r?.text)continue;const c=center(r,1000,1000);if(out.some(x=>x.text===r.text&&Math.hypot(center(x,1000,1000).x-c.x,center(x,1000,1000).y-c.y)<.08))continue;out.push(r)}return out;
}

async function scanFrame(){
  if(!running||busy)return;busy=true;frame++;const t=performance.now();
  try{
    if(!fit())return;const c=$("scanCanvas"),ctx=c.getContext("2d",{willReadFrequently:true});if(!ctx)throw Error("Canvas 2D no disponible");ctx.drawImage($("video"),0,0,c.width,c.height);
    let valid=[];
    if(scanEngine==="wasm"){
      try{
        ...
        wasmErrors++;console.error("ZXing WASM frame failed",e);
        if(wasmErrors>=2){scanEngine="js";$("scanStatus").textContent="Motor compatible iPhone activado";feedback("<b>Modo compatible activado.</b> Sigo escaneando sin perder la sesión.","warn")}
        return;
      }
    }else valid=readWithJsFallback(c);
    const batch=collect(valid,c.width,c.height);if(batch.items.length||batch.dup)addBatch(batch.items,batch.dup);
    $("scanStatus").textContent=`Activo · ${scanEngine==="wasm"?"multicódigo + invertidos":"compatible"} · ${valid.length} detectado(s) · ${Math.round(performance.now()-t)} ms`;
  }catch(e){console.error(e);$("scanStatus").textContent="Error: "+String(e?.message||e).slice(0,90)}
  finally{busy=false;if(running)timer=setTimeout(scanFrame,scanEngine==="wasm"?55:80)}
}
...
async function start(){
  ...
  try{
    ...
    if(scanEngine==="js"&&!ensureJsReader()){feedback("No se ha podido cargar ningún motor de lectura. Recarga la página con conexión.","bad");return}
