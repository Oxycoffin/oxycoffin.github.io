...
const cacheName=/const CACHE_NAME\s*=\s*['"]([^'"]+)/.exec(source)[1];
function fixture(){
...
test('app shell caches every versioned script and stylesheet in HTML',async()=>{const f=fixture();await f.event('install');const html=fs.readFileSync(require('node:path').join(__dirname,'../index.html'),'utf8');const assets=[...html.matchAll(/(?:src|href)="([^"\s]+\?v=\d+)"/g)].map(x=>x[1]);assert.equal(assets.length,4);for(const asset of assets)assert.ok(f.stores.get(cacheName).has(asset),asset);});
